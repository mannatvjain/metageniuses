...
├── configs/extraction/               # run configs (default + tiny local test)
...
- `configs/extraction/default.json`: default extraction config (points to curated forward-pass dataset)
...
3. Keep generated activations in `data/activations/` (ignored by git).
4. Keep test run artifacts in `data/test-activations/` (ignored by git).
...
4. Manifest + loader contract for later SAE module consumption
5. No-download fake adapter path for reliable local testing
